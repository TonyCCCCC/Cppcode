...
const uint_32 N=200;
const double X_L=-1,X_R=1,Y_Low=-1,Y_Upp=1,T=0.5,alpha=0.5;
...
int main()
{
   ...
   double t,*xarry=new double[M],*yarray=new double[Ms];
	...
	double mh1,mh2,tmalpha=3-alpha,oneoverh1s=1/(h1*h1),oneoverh2s=1/(h2*h2),tq,tp3ma;
	double x_s=X_L+0.5*h1,x_e=X_L+(M+0.5)*h1,y_s=Y_Low+0.5*h2,y_e=Y_Low+(M+0.5)*h2;
	double oneoh1sxl=oneoverh1s*X_L,oneoh1sxr=oneoverh1s*X_R,oneoh2syl=oneoverh2s*Y_Low,oneoh2syu=oneoverh2s*Y_Upp;
	...
	for (n=0;n<N;n++)
	{
		...
		for (j=0;j<M;j++)
		{
			...
			{
				mh1=X_L+(i+1)*h1;
				rhs[n][indy+i].r=tp3ma*exactsol[indy+i]*fovergama4ma-expxy[indy+i]*tq*(xarry[i]+yarray[j]);
				rhs[n][indy+i].i=0;
			}
